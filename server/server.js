    ...
    fetch(url, { signal: controller.signal })
      ...
      .then(data => {
        ...
        const movie = {
          imdbID: data.imdbID,
          Title: data.Title,
          Year: isNaN(parseInt(data.Year, 10)) ? null : parseInt(data.Year, 10),
          Released: data.Released,
          Runtime: isNaN(runtime) ? 0 : runtime,
          Genres: data.Genre ? data.Genre.split(",").map((item) => item.trim()) : [],
          Directors: data.Director ? data.Director.split(",").map((item) => item.trim()) : [],
          Writers: data.Writer ? data.Writer.split(",").map((item) => item.trim()) : [],
          Actors: data.Actors ? data.Actors.split(",").map((item) => item.trim()) : [],
          Plot: data.Plot,
          Poster: data.Poster,
        };
      ...
      .catch((err) => {
        ...
        if (err.name === 'AbortError') {
          console.error('OMDb API request timeout:', err);
